...
const admins = [process.env.adminId]; //TODO: сделать проверку админов через бд
...
bot.command("get", async (ctx) => {
  ...
  waitUntil(
    fetch(`https://${process.env.WEBHOOK_URL}/get?chat_id=${chat_id}`,{headers: {'protection-secret': process.env.REQUEST_SECRET}})
      .then(res => res.json())
...
bot.command('sendto', async (ctx) => {
  if (!admins.includes(String(ctx.from.id))) return; //TODO: сделать проверку админов через бд
